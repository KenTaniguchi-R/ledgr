...
export function registerRecurringTools(server: McpServer, householdId: string) {
  server.registerTool(
    "get_upcoming_bills",
    {
      ...
      description:
        "Get upcoming recurring bills sorted by next due date.",
      inputSchema: {
    ...
    async (args) => {
      ...
      const result = bills.map((b) => ({
        ...
        averageAmountDisplay: b.averageAmount !== null
          ? centsToDisplay(b.averageAmount)
          : null,
        lastAmountCents: b.lastAmount,
        lastAmountDisplay: b.lastAmount !== null
          ? centsToDisplay(b.lastAmount)
          : null,
